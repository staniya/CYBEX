...
MESSAGES['welcomeunique'] = config['MESSAGES']['welcome_special']
...
# Rooms
CHAT_ROOM = -291541520  # Cybex Chat Group
CYBEX_ADMIN = -290951152  # Administrators
TEST_ROOM = -252708624  # random test
CYBEX_LOCKED = -280403559  # Cybex Bot Lab2. This is a special group only allowed through invitation
...
ROOM_ID_TO_NAME = {
    ...
    TEST_ROOM: 'random test',
    CYBEX_LOCKED: 'Cybex Bot Lab2',
    WP_FEED: 'CYBEX Feed channel',
    SP_FEED: 'Shitpool Feed channel'
}

# Rooms where chat/gifs/etc is logged for stats etc
LOG_ROOMS = [CHAT_ROOM, TEST_ROOM]
...
def start(bot, update):
    ...
    if (update.message.chat.type == 'group') or (update.message.chat.type == 'supergroup'):
        ...
        bot.sendMessage(chat_id=chat_id, text=msg, reply_to_message_id=message_id, parse_mode="Markdown",
        ...
        msg = bot.sendMessage(chat_id=chat_id, text=(MESSAGES['start'] % name), parse_mode="Markdown",
                              disable_web_page_preview=1)

        if user_id in ADMINS:
            msg = bot.sendMessage(chat_id=chat_id, text=(MESSAGES['admin_start'] % name), parse_mode="Markdown",
                                  disable_web_page_preview=1)
...
def exchanges(bot, update):
    ...
    if (update.message.chat.type == 'group') or (update.message.chat.type == 'supergroup'):
        ...
        bot.sendMessage(chat_id=chat_id, text=msg, parse_mode="Markdown", disable_web_page_preview=1)
    # bot.forwardMessage(chat_id=CYBEX_ADMIN, from_chat_id=chat_id, message_id=message_id)
...
def donation(bot, update):
    ...
    if (update.message.chat.type == 'group') or (update.message.chat.type == 'supergroup'):
        ...
        bot.sendPhoto(chat_id=chat_id, photo="AgADBAADlasxG4uhCVPAkVD5G4AaXgtKXhkABL8N5jNhPaj1-n8CAAEC",
                      caption="Donations by bitcoin to: 175oRbKiLtdY7RVC8hSX7KD69WQs8PcRJA")
...
def topstickers(bot, update):
    ...
    bot.sendMessage(chat_id=update.message.chat_id,
                    text="Posting... sometimes this can cause the telegram api to 'time out' ? so won't complete posting but trying anyway..")
    bot.sendMessage(chat_id=CHAT_ROOM, text="CYBEX most popular 3 stickers in the last 3 days are...")
    for g in gifs:
        pprint(g)
        bot.sendMessage(chat_id=CHAT_ROOM, text="with " + str(g['total']) + " posts..")
        bot.sendSticker(chat_id=CHAT_ROOM, sticker=g['_id'], disable_notification=False)
        time.sleep(5)
    bot.sendMessage(chat_id=update.message.chat_id, text="message has been posted to " + ROOM_ID_TO_NAME[CHAT_ROOM])
...
def topgif(bot, update):
    ...
    bot.sendMessage(chat_id=CHAT_ROOM,
                    text="CYBEX most popular gif ever with " + str(gifs[0]['total']) + " posts is...")
    bot.sendSticker(chat_id=CHAT_ROOM, sticker=gifs[0]['_id'], disable_notification=False)
    bot.sendMessage(chat_id=chat_id, text="message has been posted to " + ROOM_ID_TO_NAME[CHAT_ROOM])
...
def topgifposters(bot, update):
    ...
    users = list(db.gifs.aggregate(pipe))

    msg = "I'm naming and shaming the top 5 gif posters in " + ROOM_ID_TO_NAME[CHAT_ROOM]
    ...
    msg = bot.sendMessage(chat_id=CHAT_ROOM, text=msg)
    bot.forwardMessage(chat_id=chat_id, from_chat_id=CHAT_ROOM, message_id=msg.message_id)
    bot.sendMessage(chat_id=chat_id, text="message has been posted to " + ROOM_ID_TO_NAME[CHAT_ROOM])
...
def todayinwords(bot, update):
    ...
    msg = bot.sendPhoto(chat_id=CHAT_ROOM, photo=open(PATH_WORDCLOUD, 'rb'), caption="Today in a picture")
    bot.sendMessage(chat_id=chat_id, text="Posted today in pictures to " + ROOM_ID_TO_NAME[CHAT_ROOM])
...
def todaysusers(bot, update):
    ...
    msg = bot.sendPhoto(chat_id=CHAT_ROOM, photo=open("telegram-usernames.png", 'rb'), caption="Todays Users")
    bot.sendMessage(chat_id=chat_id, text="Posted today in pictures to " + ROOM_ID_TO_NAME[CHAT_ROOM])
...
def promotets(bot, update):
    ...
    if len(fmsg) > 0:
        ...
        # rooms = [CHAT_ROOM]
        rooms = [CHAT_ROOM, WP_FEED, SP_FEED]

        for r in rooms:

            message = fmsg[0]
            bot.sendSticker(chat_id=r, sticker="CAADBAADcwIAAndCvAgUN488HGNlggI", disable_notification=False)
            msg = bot.sendMessage(chat_id=r, parse_mode="Markdown",
                                  text=fmsg[0] + "\n-------------------\n*/announcement from " + name + "*")

            if r in [CHAT_ROOM]:
                bot.pin_chat_message(r, msg.message_id, disable_notification=True)

            bot.sendMessage(chat_id=r, parse_mode="Markdown", text="Message me (" + BOTNAME.replace('_',
                                                                                                    '\_') + ") - to see details on how to connect to [teamspeak](https://whalepool.io/connect/teamspeak) also listen in to the listream here: livestream.whalepool.io",
                            disable_web_page_preview=True)
            bot.sendMessage(chat_id=chat_id, parse_mode="Markdown", text="Broadcast sent to " + ROOM_ID_TO_NAME[r])

    else:
        bot.sendMessage(chat_id=chat_id, text="Please include a message in quotes to spam/shill the teamspeak message")
...
def shill(bot, update):
    ...
    bot.sendMessage(chat_id=CYBEX_ADMIN, parse_mode="Markdown", text=name + " just shilled")
...
def commandstats(bot, update):
    ...
    reply = "*Management requests since the start of the month...*\n"
...
def fooCandlestick(ax, quotes, width=0.029, colorup='#FFA500', colordown='#222', alpha=1.0):
    ...
    for q in quotes:
        ...
        vline_lo = Line2D(xdata=(timestamp, timestamp), ydata=(lo, box_l), color='k', linewidth=0.5, antialiased=True,
                          zorder=10)
        vline_hi = Line2D(xdata=(timestamp, timestamp), ydata=(box_h, hi), color='k', linewidth=0.5, antialiased=True,
                          zorder=10)
        rect = Rectangle(xy=(timestamp - OFFSET, box_l), width=width, height=height, facecolor=color, edgecolor=color,
                         zorder=10)
...
def whalepooloverprice(bot, update):
    global api_timeframe, date_group_format, bar_width
    user_id = update.message.from_user.id
    chat_id = update.message.chat_id
    ...
    # Room only
    mongo_match = {"$match": {'chat_id': CHAT_ROOM}}
    ...
    ax1.set_title('CYBEX Messages, Gif & User joins per hour over price', fontsize=20, fontweight='bold')
    ax1.xaxis_date()
    ...
    msg = bot.sendPhoto(chat_id=CHAT_ROOM, photo=open(PATH_MSGS_OVER_PRICE, 'rb'),
                        caption="CYBEX Messages, Gif & User joins per hour over price")
    bot.sendMessage(chat_id=chat_id,
                    text="'CYBEX Messages, Gif & User joins per hour over price' posted to " + ROOM_ID_TO_NAME[
                        CHAT_ROOM])
...
def new_chat_member(bot, update):
    ...
    if (chat_id == CHAT_ROOM) or (chat_id == CYBEX_LOCKED):
        ...
        # Another user joined the chat
            ...
            # bot.restrict_chat_member(chat_id, user_id, until_date=(datetime.datetime.now() +
            # relativedelta(years=2)), can_send_messages=False, can_send_media_messages=False, can_send_other_
            # messages=False, can_add_web_page_previews=False)

            logger.info("welcoming - " + name)
            msg = random.choice(MESSAGES['welcome']) % name

            if chat_id == CYBEX_LOCKED:
                msg = (MESSAGES['welcomeunique'] % name)
    ...
    if chat_id == CYBEX_LOCKED:
        bot.restrict_chat_member(CYBEX_LOCKED, user_id, until_date=(datetime.datetime.now() + relativedelta(years=2)),
                                 can_send_messages=False, can_send_media_messages=False, can_send_other_messages=False,
                                 can_add_web_page_previews=False)

    if chat_id == CHAT_ROOM:
        bot.restrict_chat_member(CHAT_ROOM, user_id, until_date=(datetime.datetime.now() + relativedelta(days=14)),
                                 can_send_messages=False, can_send_media_messages=False, can_send_other_messages=False,
                                 can_add_web_page_previews=False)
...
def echo(bot, update):
    ...
    if username is not None:
        ...
        info = {'user_id': user_id, 'chat_id': chat_id, 'message_id': message_id, 'message': message,
                'timestamp': timestamp}
...
def sticker_message(bot, update):
    ...
    if chat_id:
        ...
        if username is not None:
            info = {'user_id': user_id, 'chat_id': chat_id, 'message_id': message_id, 'sticker_id': sticker_id,
                    'timestamp': timestamp}
    ...
    if chat_id in LOG_ROOMS:
        ...
        if update.message.document.mime_type in images:
            ...
            message = bot.sendMessage(chat_id=chat_id,
                                      text=name + ", I deleted that pic you just posted because it is an uncompressed format, please use the \'compress\' format when posting pictures.",
        ...
        if update.message.document.mime_type == 'video/mp4':
            ...
            if username is not None:
                info = {'user_id': user_id, 'chat_id': chat_id, 'message_id': message_id, 'file_id': file_id,
                        'timestamp': timestamp}
...
def links_and_hashtag_messages(bot, update):
    ...
    if (len(find_shill) > 0) and chat_id in ROOM_ID_TO_NAME:
        ...
        reply = 'CYBEX does not allow users to post personal affiliate links. This prevents many problems that can poison a community. CYBEX pays for all staff, servers, competitions and costs through affiliaite revenue, thus we ask users to support CYBEX by using the community affiliate links instead.  '

        for s in COUNTER_SHILL:

            found = re.findall(s['regex'], update.message.text)
            if len(found) > 0:
                reply += "\nThe user " + name + " recommended " + s['title'] + ", to signup on " + s[
                    'title'] + " please use the following link: " + s['link'] + ""

        # Send message to mod chat that soemeone has shilled
        bot.sendMessage(chat_id=CYBEX_ADMIN, text=name + " from " + ROOM_ID_TO_NAME[chat_id] + " posted a shill link",
                        parse_mode="Markdown", disable_web_page_preview=1)
    ...
    if len(furlcnt) > 0 and (chat_id == CHAT_ROOM):
        forward = WP_FEED
...
# Links & Hashtags
dp.add_handler(MessageHandler((Filters.entity(MessageEntity.HASHTAG) | Filters.entity(MessageEntity.URL)),
                              links_and_hashtag_messages))

# Documents
dp.add_handler(MessageHandler(Filters.document, document_message))

# Someone private messages Management
